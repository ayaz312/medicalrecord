...
function viewDoctorById(uint _id) public view returns(uint id, string memory name, string memory qualification, string memory workplace) {
    ...
    function viewPatientRecordById(uint256 id) public view  returns(uint pid,uint age,string memory name,string memory disease){


         for (uint i =0; i < patient_record.length; i++) 
        {
            if(patient_record[i].Id == id) {
                disease = string(patient_record[i].disease);
                return (patient_record[i].Id,patient_record[i].age,patient_record[i].name,disease);
            }
